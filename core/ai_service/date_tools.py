...
def next_refresh_date(last_date: date | None = None, date_to_refresh: int = 14) -> date:
...
def check_refresh_due(refresh_date: date, today: date | None = None) -> bool:
    if today is None:
        today = date.today()
...
def initialize_refresh_date(
    start_date: date | None = None, date_to_refresh: int = 14
) -> date:
